...
void VescToOdom::yawCallback(const std_msgs::msg::Float64::SharedPtr msg) {
  //yaw_ = msg->data;
  //yaw_ = yaw_ * (M_PI / 180.0); //convert from deg to rad
  double new_yaw = msg->data * (M_PI / 180.0); // Convert from degrees to radians

  // Declare and initialize old_yaw if it's not already declared in your class
  static double old_yaw = new_yaw;  // Initialize old_yaw with the initial value of new_yaw

  // Calculate del_yaw (change in yaw)
  yaw_ = new_yaw - old_yaw;

  // Update the yaw_ variable with the new value
  old_yaw = new_yaw;
